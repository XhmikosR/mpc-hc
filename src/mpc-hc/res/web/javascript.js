...
(function () {
    ...
    function controlsInit(_filePath, _curPos, _length, _state, _pbr, _eta, _volume, _muted, _hasAudio) {
        ...
        // we only hoist these variables to the global scope
    ...
    function positionUpdate() {
        if (window.event.keyCode < 46 || window.event.keyCode > 58 || window.event.keyCode === 47) {
            return false;
        }
        setTimeout(function() {
    ...
    // we only hoist these functions to the global scope
    window.controlsInit = controlsInit;
    window.update = update;
    window.timeSyntax = timeSyntax;
    window.secondsToTS = secondsToTS;
    window.postForm = postForm;
    window.positionUpdate = positionUpdate;
    ...
    function onReadyStateChange() {
        ...
        if (httpRequestStatus && httpRequestStatus.readyState === 4 && httpRequestStatus.responseText) {
            if (httpRequestStatus.responseText.charAt(0) !== "<") {
                ...
                window.alert(httpRequestStatus.responseText);
            }
    ...
    // we only hoist these functions to the global scope
    window.onLoadSnapshot = onLoadSnapshot;
    window.onAbortErrorSnapshot = onAbortErrorSnapshot;
